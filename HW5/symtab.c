...
struct SymNode* createVarNode( const char *name, int scope, struct PType *type, int localVarNum) 
{
...
struct SymNode* createParamNode( const char *name, int scope, struct PType *type, int localVarNum )
{
	struct SymNode *newNode = (struct SymNode *)malloc( sizeof(struct SymNode) );
	/* setup name */
	newNode->name = (char *)malloc(sizeof(char)*(strlen(name)+1));
