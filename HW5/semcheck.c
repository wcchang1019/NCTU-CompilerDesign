...
void verifyUnaryNOT( struct expr_sem *expr, struct SymTable *table, int scope, char *fileName )
{
	...
	if( expr->pType->dimNum != 0 ) {
		...
		genUnaryNOT(expr, table, scope, fileName);
...
void verifyRelOp( struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2 , struct SymTable *table, int scope, char *fileName, int count)
{
	if( (op1->pType->type)==ERROR_t || (op2->pType->type)==ERROR_t ) {
		op1->pType->type = ERROR_t;
	}
		...
		genRelationOp(op1, operator, op2, table, scope, fileName, count);
		op1->pType->type = BOOLEAN_t;
	}
}

void verifyAndOrOp( struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2 , struct SymTable *table, int scope, char *fileName)
{
	if( (op1->pType->type)==ERROR_t || (op2->pType->type)==ERROR_t ) {
		op1->pType->type = ERROR_t;
	}
		...
		genAndOrOp(op1, operator, op2, table, scope, fileName);
		op1->pType->type = BOOLEAN_t;
	}
}
...
struct expr_sem *verifyFuncInvoke( const char *id, struct expr_sem *exprList, struct SymTable *table, int scope )
{
	...
	if( node == 0 ) {	// symbol not found
		...
		if( node->attribute->formalParam->paramNum == 0 ) {
			if( exprList != 0 ) {
				fprintf( stdout, "########## Error at Line#%d: 1too many arguments to function %s ##########\n", linenum, node->name ); semError = __TRUE;
				result->pType = createPType( ERROR_t );
			}
			else {
				//result->pType = node->type;	// return type of function declaration
				result->pType = createPType(node->type->type);
				genFunctInvoke(id, result->pType, NULL);
			}
		}
		else {
			__BOOLEAN mismatch = __FALSE;	
			struct PTypeList *listPtr;
			struct expr_sem *exprPtr, *eeee;
			...
			if( mismatch == __TRUE ) {
				...
				if( listPtr != 0 ) {
					...
					fprintf( stdout, "########## Error at Line#%d: 2too many arguments to function '%s' ##########\n", linenum, node->name ); semError = __TRUE;
					result->pType = createPType( ERROR_t );
				}
				else {
					//printf("obobobob\n");
					eeee=exprList;
					while(eeee != NULL){
						genOneVarLoad(eeee, table, scope, "output");
						eeee = eeee->next;
					}
					result->pType = createPType(node->type->type);
					genFunctInvoke(id, result->pType, node->attribute->formalParam->params);//segmentation fault
				}
			}
...
int insertParamIntoSymTable( struct SymTable *table, struct param_sem *params, int scope, int localVarNum)
{
			...
			if( parPtr->pType->isError == __TRUE ) { 
				...
				for( idPtr=(parPtr->idlist) ; idPtr!=0 ; idPtr=(idPtr->next) ) {
					if( verifyRedeclaration( table, idPtr->value, scope ) ==__FALSE ) { result = __TRUE;  }
						...
						newNode = createParamNode( idPtr->value, scope, parPtr->pType, localVarNum);
	...
	return localVarNum;	// __TRUE: with some error(s)
}
...
__BOOLEAN verifyFuncDeclaration( struct SymTable *table, struct param_sem *par, struct PType *scalarType, struct SymNode *node ){
	...
	if( node->isFuncDefine == __TRUE ){
		...
		if( scalarType->type != node->type->type ){
			...
			if( par != 0 ) {
				fprintf( stdout, "########## Error at Line#%d: 3too many arguments to function %s ##########\n", linenum, node->name ); semError = __TRUE;
				result = __FALSE;				
			}
			else{
			...
			if( mismatch == __TRUE ) {
				...
				if( listPtr != 0 ) {
					...
					fprintf( stdout, "########## Error at Line#%d: 4too many arguments to function '%s' ##########\n", linenum, node->name ); semError = __TRUE;
					result = __FALSE;
				}
				else{
					node->isFuncDefine = __TRUE;
