...
int scope = 0;
int localVarNum = 1;
int relationCount = 0, ifCount = 0, whileCount = 0, forCount = 0;
char* fname = "output";
...
int inloop = 0;
bool varLoad1 = false, varLoad2 = false;
bool readyReturn = false;
bool inMain = false;
int genIfQueue[150];
int genIfQueueCount = 0;
int genForQueue[150];
int genForQueueCount = 0;
int genWhileQueue[150];
int genWhileQueueCount = 0;
char *forFileName;
%}
...
funct_def : scalar_type ID L_PAREN R_PAREN 
			{
				...
				genFunctBegin($2, NULL, $1);
			}
			compound_statement 
			{
				if(!readyReturn || inMain) genFunctNoReturn();
				readyReturn = false; 
				funcReturn = 0; 
				genFunctEnd();
				inMain = false;
				...
				// check and insert function into symbol table
					...
					if( node != 0 ){
						if(verifyFuncDeclaration( symbolTable, $4, $1, node ) == __TRUE){	
							localVarNum = insertParamIntoSymTable( symbolTable, $4, scope+1, localVarNum);
						}				
					}
					else{
						localVarNum = insertParamIntoSymTable( symbolTable, $4, scope+1, localVarNum );				
		  ...
		  | VOID ID L_PAREN R_PAREN 
			{
				...
				genFunctBegin($2, NULL, funcReturn);
				...
				for( ptr=$2 ; ptr!=0 ; ptr=(ptr->next) ) {						
					if( verifyRedeclaration( symbolTable, ptr->para->idlist->value, scope ) == __FALSE ) { }
						...
						if( verifyVarInitValue( $1, ptr, symbolTable, scope ) ==  __TRUE ){	
							if(scope == 0){
								genGlobalVariables(ptr->para->idlist->value, ptr->para->pType);
								...
								if(ptr->isInit == __TRUE){
									genLocalVarAssign(localVarNum);
								}
								newNode = createVarNode( ptr->para->idlist->value, scope, ptr->para->pType, localVarNum);
								localVarNum++;
							}
...
simple_statement : variable_reference ASSIGN_OP logical_expression SEMICOLON
					{
						...
						if(!(varLoad1 || varLoad2)){
							genOneVarLoad($3, symbolTable, scope, fname);
						}
						genVarStore($1, symbolTable, scope, fname);
						varLoad1 = false;
						varLoad2 = false;
					}
				 | PRINT {genPrint1();}logical_expression SEMICOLON 
				 { 
				 	verifyScalarExpr( $3, "print" ); 
				 	if(!$3->isFunctInvoke){
					 	if(!(varLoad1 && varLoad2)) {genPrint2($3, symbolTable, scope, fname, 0);}
					 	else{
					 		genPrint2($3, symbolTable, scope, fname, 1);
					 	}
					}else{
							genPrint2($3, symbolTable, scope, fname, 1);
					}
				 ...
				 | READ variable_reference SEMICOLON 
					{ 
						if( verifyExistence( symbolTable, $2, scope, __TRUE ) == __TRUE ){						
							...
							genRead($2, symbolTable, scope, fname);
						}
					}
				 ;

conditional_statement : IF L_PAREN conditional_if  R_PAREN compound_statement{genOnlyIfEnd(genIfQueue[genIfQueueCount-1]); genIfQueueCount--;}
					  | IF L_PAREN conditional_if  R_PAREN compound_statement{genElseBegin(genIfQueue[genIfQueueCount-1]);}
						ELSE compound_statement{genElseEnd(genIfQueue[genIfQueueCount-1]); genIfQueueCount--;}
					  ;
conditional_if : logical_expression 
				{ 
					verifyBooleanExpr( $1, "if" ); 
					genIfBegin(ifCount);
					genIfQueue[genIfQueueCount] = ifCount;
					genIfQueueCount++;
					ifCount++;
				};				  
...
while_statement : WHILE 
				{	
					genWhileBegin(whileCount);
					genWhileQueue[genWhileQueueCount] = whileCount;
					whileCount++;
					genWhileQueueCount++;
				}
				L_PAREN logical_expression 
				{ 
					genWhileConditionBegin(genWhileQueue[genWhileQueueCount-1]);
				...
				{ 
					inloop--; 
					genWhileEnd(genWhileQueue[genWhileQueueCount-1]);
					genWhileConditionEnd(genWhileQueue[genWhileQueueCount-1]); 
					genWhileQueueCount--;
				}
...
for_statement : FOR L_PAREN initial_expression SEMICOLON
					{
						genForBegin(forCount);
						genForQueue[genForQueueCount] = forCount;
						forCount++;
						genForQueueCount++;
					} 
					control_expression 
					{ 
						genForConditionBegin(genForQueue[genForQueueCount-1]);
					}
					SEMICOLON 
					{
						int size = snprintf( NULL, 0, "tmp_for%d", genForQueue[genForQueueCount-1]);
						forFileName = (char*)malloc(size++);
						snprintf(forFileName, size, "tmp_for%d", genForQueue[genForQueueCount-1]);
						fpFor = fopen(forFileName, "w");
						tmp = fpOut; 
						fpOut = fpFor; 
						fpFor = tmp;
					}
					...
					compound_statement  
					{
					    ...
					    fclose(fpFor);
						inloop--; 
						genForEnd(genForQueue[genForQueueCount-1]);
						genForConditionEnd(genForQueue[genForQueueCount-1]);
						genForQueueCount--;
					}
			  ;
			   ...
			   | RETURN logical_expression SEMICOLON
				{
					...
					if($2->next == NULL) 
					{
						genOneVarLoad($2, symbolTable, scope, fname);
						//printf("sdfsdf\n");
					}
					if(!inMain) genFunctReturn(funcReturn);
					readyReturn = true;
				}
			   ;
...
logical_expression : logical_expression OR_OP logical_term
					{
						verifyAndOrOp( $1, OR_t, $3, symbolTable, scope, fname );
						$$ = $1;
						$$->isCombine = true;
					}
				   | logical_term { $$ = $1; }
...
logical_term : logical_term AND_OP logical_factor
				{
					verifyAndOrOp( $1, AND_t, $3, symbolTable, scope, fname);
					$$ = $1;
					$$->isCombine = true;
				}
			 | logical_factor { $$ = $1; }
...
logical_factor : NOT_OP logical_factor
				{
					verifyUnaryNOT( $2, symbolTable, scope, fname);
					$$ = $2;
				}
...
relation_expression : arithmetic_expression relation_operator arithmetic_expression
					{
						verifyRelOp( $1, $2, $3, symbolTable, scope, fname, relationCount);
						relationCount++;
						$$ = $1;
						$$->isCombine = true;
					}
...
arithmetic_expression : arithmetic_expression add_op term
			{
				...
				/*if($1->isMul || varLoad1) genOneVarOperateLoad($3, $2, symbolTable, scope, fname, 0);
				else if($3->isMul || varLoad1){
					if($2 == SUB_t) genOneVarOperateLoad($1, $2, symbolTable, scope, fname, 1);
					else genOneVarOperateLoad($1, $2, symbolTable, scope, fname, 0);
				}else genTwoVarOperateLoad($1, $3, $2, symbolTable, scope, fname);*/
				genArithmeticOp($1, $2, $3, symbolTable, scope, fname);
				varLoad1 = true;
				$$ = $1;
				$$->isMul = false;
				$$->isCombine = true;
...
term : term mul_op factor
		{
			...
			/*if(varLoad2) genOneVarOperateLoad($3, $2, symbolTable, scope, fname, 0);
			else genTwoVarOperateLoad($1, $3, $2, symbolTable, scope, fname);*/
			genArithmeticOp($1, $2, $3, symbolTable, scope, fname);
			varLoad2 = true;
			$$ = $1;
